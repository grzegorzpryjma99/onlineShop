...
type MultiselectFieldProps<T, U, B> = ({
    type: 'SIMPLE'
...
export const MultiselectField = <T, U, B>(props: MultiselectFieldProps<T, U, B>) => {

    return <FormField formik={props.formik} fieldName={props.fieldName} className={props.className}
                      label={props.label}>
        <MultiSelect id={props.fieldName} options={props.options}
                     ...
                     onChange={e => {
                         props.formik.setFieldValue(props.fieldName, e.value)
                         if (props.onChange) {
